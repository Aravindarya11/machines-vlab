...
function initZpfGraph() {
    ...
    const gd = document.getElementById('zpfGraph');
    ...
    gd.on('plotly_hover', function(data) {
        if (data && data.points && data.points.length > 0) {
            ...
            coordBox.innerHTML = `<span><strong>Trace:</strong> ${name}</span> | <span><strong>Field Current (If):</strong> ${x.toFixed(3)} A</span> | <span><strong>Voltage:</strong> ${y.toFixed(1)} V</span>`;
        }
    });
...
function setDrawingTool(tool) {
    ...
    if (tool === 'drawline') {
        ...
        Plotly.relayout('zpfGraph', { dragmode: 'drawline' });
        ...
        Plotly.relayout('zpfGraph', { dragmode: 'eraseshape' });
    } else {
        if (btnZoom) btnZoom.className = 'btn btn-primary';
        if (statusText) statusText.innerHTML = 'Status: Zoom/Pan mode active.';
        Plotly.relayout('zpfGraph', { dragmode: 'zoom' });
    }
}
...
function showTempMarker(x, y) {
    ...
    Plotly.addTraces('zpfGraph', traceTempMarker);
}
...
function clearTempMarker() {
    ...
    if (idx !== -1) {
        Plotly.deleteTraces('zpfGraph', [idx]);
    }
}

// Clear all custom drawings
function clearAllDrawings() {
    Plotly.relayout('zpfGraph', { shapes: [] });
    firstClickPoint = null;
    clearTempMarker();
    const statusText = document.getElementById('drawingStatusText');
    if (statusText) {
